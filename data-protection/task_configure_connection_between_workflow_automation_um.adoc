...
[NOTE]
====
WFA 5.1 には「 WFA pack for managing Clustered Data ONTAP 」が含まれています。したがって、以前のようにこのパックを NetAppStorage Automation Store からダウンロードして、 WFA サーバに個別にインストールする必要はありません。  https://["ONTAP を管理するための WFA パック"]
...
http://["ネットアップのマニュアル： OnCommand Workflow Automation （現在のリリース）"]
