...
NOTE: MySQL のデフォルトのポート 3306 は、 VMware vSphere システムに Unified Manager をインストールする際に localhost にのみ使用できます。これは、前の構成を維持したままのアップグレードシナリオには影響しません。この設定を変更し、メンテナンスコンソールから他のホストに接続を提供することができます。
