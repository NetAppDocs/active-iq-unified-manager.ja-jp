...
[NOTE]
====
VMware vSphere システム上の Unified Manager OVA は、内部で Debian OS 13（Trixie）を実行します。サポートされている ESXi サーバーのバージョンがサポートできる仮想マシンハードウェアのバージョンについては、VMware のドキュメントを参照してください。
