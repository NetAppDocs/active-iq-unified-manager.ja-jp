...
+
.. 次のコマンドを実行して、コンテキストをmysqld_db_t`に設定します。semanage fcontext -a-t mysqld_db_t」/opt/NetApp/data「`restorecon -R -v /opt/NetApp/data`
...
+
.. 次のコマンドを実行して、コンテキストをmysqld_db_t`に設定します。semanage fcontext -a-t mysqld_db_t」/opt/netapp「`restorecon -R -v /opt/netapp
