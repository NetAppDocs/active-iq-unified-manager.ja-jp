...
+
Red Hat Enterprise Linux 8 システムの場合、JDK 17 - OpenJDK、p7zip、および AppStream リポジトリによって提供されるその他のソフトウェア パッケージを含む内部リポジトリがある場合は、AppStream リポジトリを無効にして、MySQL Community Server をインストールする必要があります。次のコマンドを実行します。
