...
Unified Manager は、WildFly ウェブサーバーにデプロイされます。WildFly 27.0.1.Final は Unified Manager にバンドルされ、設定されています。
...
* MySQL Community Edition バージョン 8.4.10 （MySQL リポジトリより）
* OpenJDK バージョン 17.0.14+6 (Red Hat Extra Enterprise Linux Server リポジトリから)
* Python 3.13.3 (Red Hat Extra Enterprise Linux Server リポジトリから)
* 7zip バージョン 7z2301-linux-x64。詳細については、 link:task_configure_7zip_package.html["7-zipソフトウェアパッケージを構成する"]。
