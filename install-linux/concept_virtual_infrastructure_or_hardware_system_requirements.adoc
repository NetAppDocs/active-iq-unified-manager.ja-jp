...
* /opt/NetApp/data ディレクトリに割り当てられた 100 GB の空きディスク容量。このディレクトリは LVM ドライブまたはターゲットシステムに接続された別のローカルディスクにマウントされます


[NOTE]
====
個別にマウントされたディレクトリ '/opt/'var/log' の場合 '/opt/に 15 GB ' /var/log' に 16 GB の空き領域があることを確認します「 /tmp` 」ディレクトリには、 10 GB 以上の空き領域が必要です。
...
さらに、Unified Managerの2つ目のインスタンスをインストールする前に、1つのUnified Managerインスタンスが監視できるノード数には制限があります。ノード制限、CPU、メモリの推奨事項については、 https://www.netapp.com/pdf.html?item=/media/13504-tr4621.pdf["Unified Manager ベストプラクティスガイド"^]を参照してください。
