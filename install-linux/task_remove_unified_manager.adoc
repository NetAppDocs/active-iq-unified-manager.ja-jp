...
+
この手順では、関連付けられているすべてのNetApp RPM パッケージが削除されます。Java、MySQL、7zip などの前提条件となるソフトウェア モジュールは削除されません。

. *オプション:* 必要に応じて、Java、MySQL、7zip などのサポート ソフトウェア モジュールを削除します。
+
`rpm -e mysql-community-client mysql-community-server mysql-community-common mysql-community-libs java-x.y`
